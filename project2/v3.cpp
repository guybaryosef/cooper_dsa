...
array< array< Data *, 50>, 65536> arr2;
array< array< Data *, 50>, 65536> arr3;
...
void sortDataList(list<Data *> &l, int field) {
  ...
  // field equals 4
    ...
    for (it = l.begin(); it != l.end() ; ++it) {
      curval = (int)(((*it)->val4[4] << 8) | (*it)->val4[5]);
    ...
    for (i = 8481 ; i < 32639 ; ++i ) {
      ...
      for (j = 0 ; j < buf1count ; ++j) {
        curval = (int)((arr1[i][j]->val4[2] << 8) | arr1[i][j]->val4[3]);
    ...
    for (i = 8481 ; i < 32639 ; ++i ) {
      bufcount = counterArr[i];
      for (j = 0 ; j < bufcount ; ++j) {
        curval = (int)((arr[i][j]->val4[0] << 8) | arr[i][j]->val4[1]);
